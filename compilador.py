...
list_reserved = ['ostende', '::', ';', '=', '+', '-', '/', '*', 'si', 'alium', ' ','^',
                 'dum', 'initus', 'et', 'uel', '<', '>', '==', '!', '(', ')', 'verum', 'falsus', ".", "reditus", "afficium", ","]
        ...
        if((child1[1] in ['BOOL', 'INT']) and (child2[1] in ['BOOL', 'INT'])):
            if(self.value in ['+', '-', '/', '*', '^']):
                ...
                if(self.value == "+"):
                    ...
                    return np.power(child1[0], child2[0]), "INT"
...
class BoolVal(Node):
    def __init__(self, value):
        self.children = None
        if(value.lower() == "falsus" or value == 0):
            self.value = 0
        else:
            self.value = 1
...
class Tokenizer:
    ...
    def selectNext(self):
        ...
        while goAgain:
            ...
            if(self.position > len(self.origin)):
            ...
            elif(self.origin[self.position-1:self.position+7] == "pelagus?"):
                self.actual.Type = "PROGOPEN"
                self.actual.value = self.origin[self.position -
                                                1:self.position+7]
                self.position += 7
            elif(self.origin[self.position-1:self.position+5] == "?prope"):
                self.actual.Type = "PROGCLOSE"
                self.actual.value = self.origin[self.position -
                                                1:self.position+5]
                self.position += 5

            elif((self.origin[self.position-1:self.position+4]).lower() == "verum"):
                self.actual.Type = "BOOL"
                self.actual.value = self.origin[self.position -
                                                1:self.position+4]
                self.position += 4

            elif((self.origin[self.position-1:self.position+5]).lower() == "falsus"):
                self.actual.Type = "BOOL"
                self.actual.value = self.origin[self.position -
                                                1:self.position+5]
                self.position += 5
            ...
            elif(str.lower(self.origin[self.position-1:self.position+6]) == "ostende"):
                self.actual.Type = "ECHO"
                self.actual.value = ("print")
                self.position += 6
            elif(str.lower(self.origin[self.position-1:self.position+7]) == "officium"):
                self.position += 8
                init = self.position
            ...
            elif(str.lower(self.origin[self.position-1:self.position+6]) == "reditus"):
                ...
                self.actual.value = (self.origin[self.position-1])
            elif(str.lower(self.origin[self.position-1:self.position+4]) == "alium"):
                self.actual.Type = "ELSE"
                self.actual.value = (self.origin[self.position-1])
                self.position += 4
            elif(str.lower(self.origin[self.position-1:self.position+1]) == "si"):
                self.actual.Type = "IF"
                self.actual.value = (self.origin[self.position-1])
                self.position += 1
            elif(str.lower(self.origin[self.position-1:self.position+5]) == "facite"):
                self.actual.Type = "DO"
                self.actual.value = (self.origin[self.position-1])
                self.position += 5
            elif(str.lower(self.origin[self.position-1:self.position+2]) == "dum"):
                self.actual.Type = "WHILE"
                self.actual.value = (self.origin[self.position-1])
                self.position += 2
            elif(str.lower(self.origin[self.position-1:self.position+5]) == "initus"):
                ...
                self.position += 1
            elif(str.lower(self.origin[self.position-1:self.position+2]) == "uel"):
                self.actual.Type = "OR"
                self.actual.value = (self.origin[self.position-2])
                self.position += 2
            elif(str.lower(self.origin[self.position-1:self.position+1]) == "et"):
                ...
                if(self.origin[init:end] in list_reserved):
                    raise Exception("Error, token reserved: %s" %
                                    self.origin[init+1:end])
                elif(not re.match(r'[A-Za-z]+[A-Za-z0-9_]*$', self.origin[init+2:end])):
...
class Parser:
    ...
    def parseTerm(tokens):
        ...
        elif(Parser.tokens.actual.Type == "POW"):
            term = BinOp("^")
            term.children.append(temp_value)
            term.children.append(Parser.parseTerm(tokens))
    ...
    def parseCommand(tokens):
        if(Parser.tokens.actual.Type == "ENDLINE"):
            ...
            Parser.tokens.selectNext()
            tmp1 = Parser.parseCommand(tokens)
...
if __name__ == '__main__':
    ...
    if(".lc" in file_LC):
        ...
        raise Exception("Type error: %s is not a '.php' file" % (file_LC))
